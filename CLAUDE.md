...
- Slash command file structure and headers in Markdown are documented in [3], but exact discovery can vary by release. unknown—verification needed.
- Tool names available by default (e.g., shell, editor/patch) may vary by environment. unknown—verification needed. See Test: Tools below.

CLI Quickstart
- Install and auth per [2].
- Verify install: run `claude --help` and `claude code --help` [1][5].
- Run slash commands by path: `claude .claude/commands/project/init.md` [3]. If unsupported, open the file in Claude Code and execute as a command (unknown—verification needed).

Slash Commands Conventions (doc-backed)
- Commands are Markdown files invoking a structured prompt for Claude Code [3].
- Keep commands short, explicit, and scoped; chain phases (init → plan → build → review) [3][6].
...
- Complexity budget: max 1 file changed per micro‑task unless the change is purely mechanical and low-risk (enforced via reviewer gate) [6].
...
- Reviewer includes a Revert Step (via git or patch) if gates fail [6].

Test: Slash Commands
- Run: `claude .claude/commands/project/init.md` (unknown—verification needed per CLI version). If not recognized, consult `claude --help` [1][3].

Test: Tools
- In a temp project, run a command that calls `shell` to `echo` a file and `git status`. Confirm output is summarized [4].
